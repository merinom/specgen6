...
class Vocab(object):
	...
	def __init__(self, dir, f = 'index.rdf', uri = None):
		...
		self.graph.parse(self.filename, format=util.guess_format(self.filename))
		self.terms = []
		self.uterms = []
	...
	# text.gsub(/<code>foaf:(\w+)<\/code>/){ defurl($1) } return "<code><a href=\"#term_#{term}\">foaf:#{term}</a></code>"
  	...
  	def termlist(self):
  		...
  		eg = """<div class="specterm" id="%s" about="%s" typeof="%s">
  			...
  			%s
  			<p style="padding: 0 6px 0 6px;margin: 10px 0 0 0;background: white; float: right; font-size: small;">[<a href="#%s">#</a>] <!-- %s --> [<a href="#glance">back to top</a>]</p>
  			<br/>
  			</div>"""
  		...
  		for term in self.vocab.properties:
  			...
  			contentStr = ''
  			for (row, label) in relations:
  				inverseproperty = row[0]
